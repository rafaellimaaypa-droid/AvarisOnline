...
export default class Player extends Character {
    ...
    public load(data: PlayerData, sync = false): void {
        ...
        if (this.isAdmin()) {
            if (this.name && !this.name.startsWith('[ADM]')) {
                this.name = `[ADM] ${this.name}`;
            }
            this.nameColour = '#FF0000';
        } else if (this.name && this.name.startsWith('[ADM]')) {
            this.name = this.name.replace(/^\[ADM\]\s*/i, '');
        }
    ...
    public override isAdmin(): boolean {
        let cleanName = this.name ? this.name.replace(/^\[ADM\]\s*/i, '').trim().toLowerCase() : '';
        return cleanName === 'mestre';
    }
