...
export default class Connection {
    ...
    private handleSync(data: PlayerData): void {
        ...
        if (player.instance === this.game.player.instance) this.game.player.sync();
    }
    ...
    /**
     * Handles friend list updates from the server.
     */
    ...
    private handleFriends(opcode: Opcodes.Friends, info: FriendsPacketData): void {
        switch (opcode) {
            ...
            case Opcodes.Friends.Remove: {
                delete this.game.player.friends[info.username!];
                break;
            }
    ...
    /**
     * Handles player rank updates from the server.
     */
    ...
    /**
     * Handles crafting user interface packets.
     */
    ...
    private handleInterface(info: InterfacePacketData): void {
        ...
        if (menu.isVisible() && menu.hideOnShow) menu.hide();
        else menu.show();
    }

    /**
     * Handles loot bag user interface packets.
     */
    ...
    /**
     * Handles countdown timer packets.
     */

    private handleCountdown(info: CountdownPacketData): void {
        this.game.player.setCountdown(info.time);
    }

    /**
     * Handles resource depletion and respawn updates.
     */

    private handleResource(info: ResourcePacketData): void {
        let resource = this.entities.get<Resource>(info.instance);
    ...
    /**
     * Checks whether or not the entity list can be requested from the server.
     */

    private canRequestEntityList(): boolean {
        return Date.now() - this.lastEntityListRequest > 3000;
    }
}
