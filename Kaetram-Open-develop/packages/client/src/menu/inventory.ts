...
export default class Inventory extends Menu {
    ...
    private goldCount: HTMLElement = document.querySelector('#inventory-gold-count')!;
    ...
    /**
     * Updates the total gold counter displayed in the inventory interface.
     */

    public updateGoldCount(): void {
        if (!this.goldCount) return;

        let totalGold = 0;
        this.forEachSlot((_index, slot) => {
            if (slot.dataset.key === 'gold') {
                totalGold += slot.count || 0;
            }
        });

        this.goldCount.textContent = totalGold.toLocaleString('en-US');
    }
