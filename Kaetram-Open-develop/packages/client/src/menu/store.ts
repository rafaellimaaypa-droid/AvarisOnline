...
export default class Store extends Menu {
    ...
    private cashCount: HTMLElement = document.querySelector('#store-cash-count')!;
    ...
    public constructor(
        ...
        super('#store', '#close-store', '#shop-button');
    ...
    /**
     * Returns the default data structure for the Premium Cash Shop.
     */

    public getCashShopData(): StorePacketData {
        return {
            key: 'cashshop',
            currency: 'Cash',
            items: [
                { index: 0, key: 'gold', name: 'Caixa Misteriosa', price: 100, count: 1 },
                { index: 1, key: 'book', name: 'Mudança de Nome', price: 500, count: 1 },
                { index: 2, key: 'token', name: 'Tag Customizada', price: 200, count: 1 },
                { index: 3, key: 'speedpotion', name: '2x XP Boost (1h)', price: 150, count: 1 },
                { index: 4, key: 'artistcrown', name: 'Fila Prioritária', price: 1000, count: 1 }
            ]
        };
    }

    /**
     * Updates the cash balance display in the store header dynamically.
     */

    public updateCashCount(): void {
        if (!this.cashCount)
            this.cashCount = document.querySelector('#store-cash-count')!;

        let cash = this.game?.player?.cash || 0;
        if (this.cashCount) {
            this.cashCount.textContent = cash.toLocaleString('en-US');
        }
    }
    ...
    /**
     * Sends a callback signal with the buy opcode or executes cash shop commands.
    ...
    private buy(index: number, count = 1): void {
        if (this.key === 'cashshop') {
            switch (index) {
                case 0: {
                    this.game.socket.send(Packets.Chat, ['/buy mysterybox']);
                    break;
                }
                case 1: {
                    this.game.socket.send(Packets.Chat, ['/buy namechange']);
                    break;
                }
                case 2: {
                    this.game.socket.send(Packets.Chat, ['/buy tag']);
                    break;
                }
                case 3: {
                    this.game.socket.send(Packets.Chat, [`/buy xpboost ${count}`]);
                    break;
                }
                case 4: {
                    this.game.socket.send(Packets.Chat, ['/buy priority']);
                    break;
                }
            }
            return;
    ...
    public override synchronize(): void {
        ...
        this.updateCashCount();

        if (this.storeList && (this.storeList.children.length === 0 || this.key === 'cashshop')) {
            if (this.storeList.children.length === 0) {
                this.update(this.getCashShopData());
            }
        }
    ...
    /**
     * Displays the store UI and updates the items in the store inventory.
     * If no store packet is supplied, defaults to displaying the Premium Cash Shop.
    ...
    private createStoreItem(item: SerializedStoreItem, index: number): HTMLElement {
        ...
        let symbol = this.currency === 'Cash' ? ' Cash' : this.currency === 'gold' ? 'g' : ` ${this.currency}`;
        price.textContent = `${item.price.toString()}${symbol}`;
